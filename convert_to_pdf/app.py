...
<script>
  ...
  function handleFile(file) {{
    ...
    const allowedPattern = /\\.(docx?|pptx?|xlsx?|csv|txt|odt|ods)$/i;
...
def libreoffice_convert(src: Path, out_dir: Path) -> Path:
  ...
  if result.returncode != 0:
      raise RuntimeError(result.stderr.decode("utf-8") or "LibreOffice conversion error")
...
def convert_route():
  ...
  try:
      pdf_path = libreoffice_convert(src_path, tmpdir)
